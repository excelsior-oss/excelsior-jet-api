...
import static com.excelsiorjet.api.TomcatConfig.WEBAPPS_DIR;
...
public abstract class AbstractJetTask<T extends AbstractJetTaskConfig> {
     ...
     */
    protected List<Dependency> copyDependencies(File buildDir, File mainJar) throws ExcelsiorJetApiException {
            ...
            config.getArtifacts()
                    ...
                    .forEach(a ->
                            copyDependency(a.getFile(), new File(libDir, a.getFile().getName()), buildDir,
                                    dependencies, !a.getGroupId().equals(config.groupId()))
                    )
            ;
            return dependencies;
        } catch (Exception e) {
    ...
    protected void copyTomcatAndWar() throws ExcelsiorJetApiException {
        try {
            ...
            Utils.copyFile(config.mainWar().toPath(), new File(config.tomcatInBuildDir(), WEBAPPS_DIR + File.separator + warName).toPath());
        } catch (IOException e) {
